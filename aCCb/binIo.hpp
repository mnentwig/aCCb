...
template<class T> vector<T> stream2vec(std::istream &is) {
...
template<class T> vector<T> stream2vec(std::istream &is, vector<bool> indexOp) {
	size_t nElem = binIo::details::getNumBinaryElem(is, sizeof(T));
...
template<class T> void vec2stream(std::ostream &os, const vector<T> &vec) {
...
template<class T> void vec2stream(std::ostream &os, const vector<T> &vec, const vector<bool> &indexOp) {
	// === main loop ===
	auto itIndex = indexOp.cbegin();
	const auto itIndexEnd = indexOp.cend();
	size_t ixWrite = 0;
...
template<class T> vector<T> file2vec(const string fname, const vector<bool> &indexOp) {
	std::ifstream is(fname, std::ifstream::binary);
	if (!is)
		throw runtime_error("failed to open file");
...
template<class T> void vec2file(const string &fname, const vector<T> &vec) {
...
template<class T> void vec2file(const string &fname, const vector<T> &vec, const vector<bool> &indexOp) {
	fstream os(fname.c_str(), std::ios::out | std::ifstream::binary);
	if (!os)
		throw runtime_error("failed to open file");
